...
%% Compute bias
wb = waitbar(0,'Evaluate Bias under different inter-distances and RIS size ...');
for s = 1:length(sizetest)
    ...
    for d = 1:length(dtest)
        ...
        for g = 1:G
            ...
            B_tilde(:,g) = ( zRS.'*(ZSS.*diag(ones(NRIS,1)) + diag(zRIS(:,g)))^(-1) ).';
        end
        D_true = [real(B_true).', -imag(B_true).'; imag(B_true).', real(B_true).'];
        D_tilde = [real(B_tilde).', -imag(B_tilde).'; imag(B_tilde).', real(B_tilde).'];

        x0 = (D_tilde.'*D_tilde)^(-1)*D_tilde.'*D_true*x_bar;
        data(s,d) = sqrt( (x_bar-x0).'*(x_bar-x0) );
    end
end
close(wb);
